...
function extractedError(message?: string) {
  if (!message) return ""
...
function compactError(message?: string) {
  return extractedError(message).replace(/\\n/g, "\n").replace(/\s+/g, " ").trim()
}

function loadErrorCopy(message?: string) {
  const error = compactError(message)
  if (!error) {
    return {
      title: "AgentBoard couldn't load",
      body: "This project could not be loaded. Ask chat to inspect the Beads setup for this workspace.",
    }
  }
  if (isMissingBeads(error)) {
    return {
      title: "Beads isn't set up here yet",
      body: "Initialize Beads for this project to start using the board.",
    }
  }
  if (/failed to open database|database .* not found on Dolt server|Dolt server/i.test(error)) {
    return {
      title: "Beads needs to reconnect",
      body:
        "The Beads database for this project is not available from the local Dolt server. This can happen after switching branches, restarting Beads, or opening another project.",
    }
  }
  if (/not found|enoent|command not found|is not in your PATH/i.test(error)) {
    return {
      title: "Beads command not found",
      body: "AgentBoard couldn't find the `bd` command. Install Beads, then refresh the board.",
    }
  }
  return {
    title: "AgentBoard couldn't load",
    body: "Something prevented AgentBoard from reading this project's Beads issues.",
  }
}

function technicalError(message?: string) {
  return compactError(message) || "No technical details were returned."
}
...
export function LoadingState() {
  return (
    <div class="size-full min-w-0 overflow-x-auto p-[18px]">
      <div class="grid h-full min-w-[1480px] grid-cols-5 gap-2">
        <For each={[0, 1, 2, 3, 4]}>
          {() => (
            <div class="flex min-h-0 flex-col rounded-lg bg-surface-raised-base">
              ...
              <div class="space-y-2.5 px-2.5 pb-3">
                <For each={[0, 1, 2]}>
                  {() => (
                    <div class="rounded-md bg-background-base p-3.5 shadow-xs-border-base">
...
export function SetupState(props: {
  ...
  return (
    <div class="flex size-full min-w-0 items-center justify-center px-6 pb-24">
      <Show
        when={missing()}
        fallback={
          <div class="w-full max-w-md text-center">
            ...
            <h2 class="text-20-medium text-text-strong [text-wrap:balance]">{loadErrorCopy(props.error).title}</h2>
            <p class="mx-auto mt-2 max-w-sm text-13-regular leading-relaxed text-text-weak">
              {loadErrorCopy(props.error).body}
            </p>
            <details class="group mx-auto mt-4 max-w-sm rounded-lg bg-surface-raised-base text-left text-12-regular text-text-base ring-1 ring-inset ring-border-weaker-base">
              <summary class="flex cursor-pointer list-none items-center justify-between gap-3 px-3 py-2 text-text-weak transition-colors hover:text-text-base">
                <span>Technical details</span>
                <Icon name="chevron-down" class="size-3.5 transition-transform group-open:rotate-180" />
              </summary>
              <pre class="max-h-36 overflow-auto border-t border-border-weaker-base px-3 py-2 font-mono text-11-regular leading-relaxed text-text-base">
                {technicalError(props.error)}
              </pre>
            </details>
