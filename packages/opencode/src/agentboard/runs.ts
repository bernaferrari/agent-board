...
import { collectRunArtifactsEffect } from "./artifacts"
...
function runInWorktree<T, E, R>(worktree: string, effect: Effect.Effect<T, E, R>) {
  return AppRuntime.runPromise(
    InstanceStore.Service.use((store) => store.provide({ directory: worktree }, effect) as Effect.Effect<T, E, AppServices>),
  )
}
  ...
  try {
    ...
    await runInWorktree(
      ...
      Effect.gen(function* () {
        ...
        const artifacts = collectRunArtifactsEffect(current)
        if (artifacts) yield* artifacts
...
${message.trim() || "Please review the current implementation, address any remaining problems, and summarize the result."}`
  ...
  async refreshArtifacts(runID: string) {
    ...
    const project = AgentBoardStore.getProject(run.projectID)
    if (!project) throw new Error(`AgentBoard project not found: ${run.projectID}`)
    await runInWorktree(project.worktree, collectRunArtifactsEffect(run) ?? Effect.void)
