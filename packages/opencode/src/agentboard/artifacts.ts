...
export function collectRunArtifactsEffect(run: AgentBoardRun) {
  ...
  return Effect.gen(function* () {
    ...
    const diffExit = yield* Effect.exit(Session.Service.use((session) => session.diff(sessionID)))
    if (diffExit._tag === "Success") {
      diffs = diffExit.value
      ...
      AgentBoardStore.addArtifact({
        ...
        data: { error: String(diffExit.cause) },
      })
    }

    const messagesExit = yield* Effect.exit(
      Session.Service.use((session) => session.messages({ sessionID, limit: 20 })),
    )
    if (messagesExit._tag === "Success") {
      ...
      AgentBoardStore.addArtifact({
        ...
        data: { error: String(messagesExit.cause) },
      })
    }
  })
}
...
export async function collectRunArtifacts(run: AgentBoardRun) {
  const effect = collectRunArtifactsEffect(run)
  if (!effect) return
